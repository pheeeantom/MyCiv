...
public static class TextureGenerator {		
    ...
    public static Texture2D GetCloud1Texture(int width, int height, Cell[,] cells)
	{
		...
		for (var x = 0; x < width; x++)
		{
			...
			{
				if (cells[x,y].Cloud1Value > 0.45f)
					pixels[x + y * width] = Color.Lerp(new Color(1f, 1f, 1f, 0), Color.white, cells[x,y].Cloud1Value);
				else
    ...
    public static Texture2D GetCloud2Texture(int width, int height, Cell[,] cells)
    {
        var texture = new Texture2D(width, height);
        var pixels = new Color[width * height];
        ...
        for (var x = 0; x < width; x++)
        {
            for (var y = 0; y < height; y++)
            {
                if (cells[x, y].Cloud2Value > 0.5f)
                    pixels[x + y * width] = Color.Lerp(new Color(1f, 1f, 1f, 0), Color.white, cells[x, y].Cloud2Value);
                else
                    pixels[x + y * width] = new Color(0, 0, 0, 0);
            }
        }
	...
	public static Texture2D GetBumpMap(int width, int height, Cell[,] cells)
	{
		var texture = new Texture2D(width, height);
		var pixels = new Color[width * height];
		...
		for (var x = 0; x < width; x++)
		{
			for (var y = 0; y < height; y++)
			{
				...
				if (!cells[x,y].Collidable)
				{
					pixels[x + y * width] = Color.Lerp(Color.white, Color.black, cells[x,y].HeightValue * 2);
				}
			}
		}
	...
	public static Texture2D GetHeatMapTexture(int width, int height, Cell[,] cells)
	{
		...
		for (var x = 0; x < width; x++)
		{
			for (var y = 0; y < height; y++)
			{
				...
				if ((int)cells[x,y].HeightType > 2 && cells[x,y].Bitmask != 15)
